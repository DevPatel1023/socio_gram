...
const MessagePage = () => {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen"><p className="text-gray-600">Loading inbox...</p></div>;
  if (error) return <div className="flex items-center justify-center h-screen"><p className="text-red-500">Error loading inbox.</p></div>;
        ...
        <div className="md:w-80 w-full flex flex-col bg-white border-r border-gray-200 overflow-hidden">
          ...
          <div className="flex-shrink-0 p-6 border-b border-gray-100">
            <div className="flex items-center justify-between mb-6">
              <h1 className="font-semibold text-2xl text-gray-900">{user?.username}</h1>
              <MoreHorizontal size={20} className="text-gray-600 cursor-pointer hover:text-gray-800" />
            </div>
              ...
              {inboxUsers.map(({ user, type }) => (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 mx-2 rounded-lg cursor-pointer transition-all duration-200 hover:bg-gray-50 ${
                    selectedUserInbox === user._id ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                  }`}
                >
                  <div className="relative">
          ...
          {selectedUserInbox ? (
            ...
            <div className="flex flex-col items-center justify-center h-full bg-gray-50">
              <div className="text-center">
                ...
                <p className="text-gray-500 max-w-sm">Send private photos and messages to a friend or group.</p>
              </div>
            </div>
          )}
        </div>
